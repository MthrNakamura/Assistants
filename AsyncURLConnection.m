...
-(void)performRequest {
    ...
    //NSLog( @"connection retain count: %d", [_connection retainCount] );
...
-(void)join {
    @try {
        ...
        while ( !self.synchronousComplete ) {
            @autoreleasepool {
                // 通信の完了を待つ
...
-(void)timeout {
    [self cancel];
    [self connection:_connection didFailWithError:[NSError errorWithDomain:NSURLErrorDomain
...
-(void)connectionDidFinishLoading:(NSURLConnection *)connection {
    ...
    __block AsyncURLConnection* _self = self;
    if ( completeBlock ) completeBlock( _self, data );

    self.connectionComplete = TRUE;
    self.synchronousComplete = TRUE;
    
    // retainカウントがなぜか+1されているので対応
   // [connection release];
...
-(void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error {
    ...
    __block AsyncURLConnection* _self = self;
    if ( errorBlock ) errorBlock( _self, error );
